...
export default function Home() {
  ...
  // Estados para o Formulário de Sugestão (Incluindo Spotify)
  const [mostrarSugestao, setMostrarSugestao] = useState(false);
  const [novaSugestao, setNovaSugestao] = useState({ 
    titulo: "", 
    linha: "", 
    letra: "",
    link_spotify: "" // Novo campo opcional
  });
  ...
  const enviarSugestao = async (e: React.FormEvent) => {
    ...
    try {
      ...
      alert("Saravá! Sugestão enviada para análise.");
         ...
         <div className="max-w-3xl mx-auto space-y-6">
            ...
            <div className="flex justify-between items-center">
              ...
              <button 
                ...
                className="bg-indigo-600/10 hover:bg-indigo-600 border border-indigo-500/20 hover:border-indigo-500 px-5 py-3 rounded-2xl text-[9px] font-black uppercase tracking-widest text-white transition-all active:scale-95 flex items-center gap-2"
              >
            ...
            <div className="flex gap-2 h-12">
               ...
               <select 
                 ...
                 className="bg-indigo-600/10 border border-indigo-500/30 text-indigo-400 px-4 rounded-2xl text-[10px] font-black uppercase tracking-widest outline-none min-w-[110px] text-center"
               >
        ...
        {filtrados.map(p => (
          ...
          >
            ...
            <div className="flex gap-2.5">
              {p.link_youtube && (
                <svg className="w-5 h-5 text-red-600 fill-current opacity-80" viewBox="0 0 24 24">
                  <path d="M19.615 3.184c-3.604-.246-11.631-.245-15.23 0-3.897.266-4.356 2.62-4.385 8.816.029 6.185.484 8.549 4.385 8.816 3.6.245 11.626.246 15.23 0 3.897-.266 4.356-2.62 4.385-8.816-.029-6.185-.484-8.549-4.385-8.816zm-10.615 12.816v-8l8 4-8 4z"/>
                </svg>
              )}
              {p.link_spotify && (
                <svg className="w-5 h-5 text-green-500 fill-current opacity-80" viewBox="0 0 24 24">
        ...
        <div className="fixed inset-0 z-[100] bg-black/95 backdrop-blur-lg flex items-center justify-center p-4">
          <div className="bg-[#0B1120] border border-white/10 rounded-[2.5rem] w-full max-w-lg max-h-[85vh] overflow-hidden flex flex-col shadow-2xl">
            <div className="p-8 border-b border-white/5 text-center relative">
              <button onClick={() => setPontoAberto(null)} className="absolute right-8 top-8 text-slate-500 text-xl">✕</button>
            ...
            <div className="p-8 border-t border-white/5 bg-slate-950/50 flex flex-col gap-3">
                ...
                <button 
                  ...
                  className="bg-indigo-600 text-white py-4 rounded-xl font-black text-[10px] uppercase tracking-widest shadow-lg shadow-indigo-600/20"
                >
        ...
        <div className="fixed inset-0 z-[100] bg-black/95 backdrop-blur-md flex items-center justify-center p-4">
          <form onSubmit={enviarSugestao} className="bg-[#0B1120] border border-white/10 rounded-[2.5rem] w-full max-w-md p-8 shadow-3xl flex flex-col gap-5">
            ...
            <div className="space-y-4">
              <input required placeholder="Título" value={novaSugestao.titulo} onChange={e => setNovaSugestao({...novaSugestao, titulo: e.target.value})} className="w-full bg-white/5 border border-white/10 p-4 rounded-xl text-white text-sm outline-none focus:border-indigo-500/50" />
              ...
              <div className="relative">
                <input 
                  ...
                  className="w-full bg-white/5 border border-white/10 p-4 rounded-xl text-white text-[10px] outline-none focus:border-green-500/30 transition-all placeholder:text-slate-600" 
                />
                <div className="absolute right-4 top-1/2 -translate-y-1/2 w-4 h-4 text-green-500/50 pointer-events-none">
            ...
            <button type="submit" disabled={enviando} className="bg-indigo-600 py-4 rounded-xl text-white font-black uppercase text-[10px] tracking-widest transition-all active:scale-95 disabled:opacity-50">
              {enviando ? "ENVIANDO..." : "ENVIAR FUNDAMENTO"}
            </button>
          </form>
        </div>
      )}
