...
export default function Home() {
  ...
  useEffect(() => {
    async function carregar() {
      ...
      if (pts) {
        ...
        // DETECÇÃO DE LINK COMPARTILHADO (?p=id)
  ...
  return (
    <div className="min-h-screen bg-[#020617] text-slate-200 font-sans pb-10">
         ...
         <div className="max-w-3xl mx-auto space-y-6">
            ...
            <div className="flex gap-2 h-12">
               ...
               <select 
                 ...
                 className="bg-indigo-600/10 border border-indigo-500/30 text-indigo-400 px-4 rounded-2xl text-[10px] font-black uppercase tracking-widest outline-none min-w-[120px]"
               >
      ...
      {/* LISTAGEM DE PONTOS NA TELA INICIAL */}
          ...
          <div className="bg-[#0B1120] border border-white/10 rounded-[2.5rem] w-full max-w-lg max-h-[85vh] overflow-hidden flex flex-col shadow-2xl">
            <div className="p-8 border-b border-white/5 text-center relative">
              <button 
                onClick={() => {
                  setPontoAberto(null);
                  window.history.pushState({}, "", "/"); // Remove o ?p= da URL ao fechar
                }} 
            ...
            {/* AÇÕES DE COMPARTILHAMENTO E MÍDIA */}
            <div className="p-8 border-t border-white/5 bg-slate-950/50 flex flex-col gap-3">
                <div className="grid grid-cols-2 gap-3">
                  {pontoAberto.link_youtube && (
                    <a href={pontoAberto.link_youtube} target="_blank" className="bg-red-600/10 text-red-500 border border-red-500/20 py-3.5 rounded-xl font-black text-[9px] uppercase tracking-widest text-center">YouTube</a>
                  )}
                  {pontoAberto.link_spotify && (
                    <a href={pontoAberto.link_spotify} target="_blank" className="bg-green-600/10 text-green-500 border border-green-500/20 py-3.5 rounded-xl font-black text-[9px] uppercase tracking-widest text-center">Spotify</a>
                  )}
                </div>
                
                <button 
                  onClick={() => {
                    const linkDinamico = `${window.location.origin}?p=${pontoAberto.id}`;
                    const tituloZip = `*${pontoAberto.titulo.toUpperCase()}*`;
                    const linhaZip = `_(${pontoAberto.linha})_`;
                    const mensagem = `${tituloZip}%0A${linhaZip}%0A%0A${pontoAberto.letra}%0A%0A🌐 *Confira no Curimba Digital:*%0A${linkDinamico}`;
      ...
      {/* MODAL DE SUGESTÃO (INDICAR PONTO) */}
      {mostrarSugestao && (
        <div className="fixed inset-0 z-[100] bg-black/95 backdrop-blur-md flex items-center justify-center p-4">
          <form onSubmit={enviarSugestao} className="bg-[#0B1120] border border-white/10 rounded-[2.5rem] w-full max-w-md p-8 shadow-3xl flex flex-col gap-5">
            ...
            <input 
              ...
              className="w-full bg-white/5 border border-white/10 p-4 rounded-xl text-white text-[10px] outline-none focus:border-green-500/30" 
            />

            <textarea required rows={5} placeholder="Letra completa..." value={novaSugestao.letra} onChange={e => setNovaSugestao({...novaSugestao, letra: e.target.value})} className="w-full bg-white/5 border border-white/10 p-5 rounded-2xl text-white italic text-sm outline-none focus:border-indigo-500/50 resize-none" />
