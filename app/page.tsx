...
export default function Home() {
  ...
  return (
    <div className="min-h-screen bg-[#020617] text-slate-200 font-sans pb-10">
      <header className="sticky top-0 z-[80] bg-[#020617]/90 backdrop-blur-2xl border-b border-white/5 px-6 py-6">
         <div className="max-w-3xl mx-auto space-y-8">
            <div className="flex justify-between items-center">
              ...
              <div className="flex gap-3">
                <button onClick={() => setMostrarExtra(true)} className="bg-white/5 border border-white/10 px-5 py-3 rounded-2xl text-[10px] font-black uppercase text-slate-400 hover:text-white transition-all">Melhorias</button>
                {/* NOME DO BOTÃO CORRIGIDO */}
                <button onClick={() => setMostrarSugestao(true)} className="bg-indigo-600 px-6 py-3 rounded-2xl text-[10px] font-black uppercase text-white shadow-lg shadow-indigo-600/30 active:scale-95 transition-all">Indicar Ponto +</button>
              </div>
            </div>
            <div className="flex gap-3 h-14">
               <input placeholder="Procurar fundamento..." value={busca} onChange={e => setBusca(e.target.value)} className="flex-1 bg-white/[0.04] border border-white/10 px-6 rounded-2xl outline-none text-base focus:border-indigo-500/50" />
               <select value={filtroLinha} onChange={e => setFiltroLinha(e.target.value)} className="bg-indigo-600/10 border border-indigo-500/30 text-indigo-400 px-4 rounded-2xl text-[11px] font-black uppercase outline-none">
      ...
      <main className="max-w-3xl mx-auto px-6 mt-8 grid gap-3">
        {filtrados.map(p => (
          <button key={p.id} onClick={() => setPontoAberto(p)} className="group bg-white/[0.02] border border-white/5 p-5 rounded-3xl text-left hover:bg-white/[0.05] hover:border-indigo-500/40 transition-all flex justify-between items-center">
            <div>
              <span className="text-indigo-500 text-[8px] font-black uppercase block mb-1 opacity-80">{p.linha}</span>
              <h3 className="font-bold text-slate-100 text-lg tracking-tight">{p.titulo}</h3>
            </div>
            <div className="flex gap-3">
              {p.link_youtube && (
                <svg className="w-6 h-6 text-red-600 fill-current opacity-90" viewBox="0 0 24 24"><path d="M19.615 3.184c-3.604-.246-11.631-.245-15.23 0-3.897.266-4.356 2.62-4.385 8.816.029 6.185.484 8.549 4.385 8.816 3.6.245 11.626.246 15.23 0 3.897-.266 4.356-2.62 4.385-8.816-.029-6.185-.484-8.549-4.385-8.816zm-10.615 12.816v-8l8 4-8 4z"/></svg>
              )}
              {p.link_spotify && (
                <svg className="w-6 h-6 text-green-500 fill-current opacity-90" viewBox="0 0 24 24"><path d="M12 0C5.373 0 0 5.373 0 12s5.373 12 12 12 12-5.373 12-12S18.627 0 12 0zm5.508 17.302c-.219.359-.687.474-1.046.255-2.898-1.771-6.547-2.171-10.843-1.187-.412.095-.824-.162-.919-.573-.094-.412.163-.824.573-.919 4.707-1.074 8.745-.614 12.02 1.39.359.22.474.688.255 1.046l-.04.089zm1.47-3.253c-.276.449-.863.593-1.312.317-3.317-2.039-8.373-2.634-12.298-1.442-.505.153-1.036-.133-1.189-.638-.153-.505.133-1.036.638-1.189 4.49-1.362 10.063-.699 13.884 1.649.449.276.593.863.317 1.312l-.04.091zm.126-3.41c-3.98-2.362-10.539-2.578-14.331-1.425-.61.185-1.251-.167-1.436-.777-.185-.61.167-1.251.777-1.436 4.354-1.321 11.583-1.066 16.155 1.646.549.326.73 1.039.404 1.588-.326.549-1.039.73-1.588.404l-.001-.001z"/></svg>
              )}
            </div>
          </button>
        ))}
        ...
        <div className="fixed inset-0 z-[100] bg-black/95 backdrop-blur-xl flex items-center justify-center p-4">
          <div className="bg-[#0B1120] border border-white/10 rounded-[2.5rem] w-full max-w-lg max-h-[85vh] overflow-hidden flex flex-col shadow-2xl">
            <div className="p-8 border-b border-white/5 text-center relative">
              ...
              <span className="text-indigo-500 text-[10px] font-black uppercase mb-1 block tracking-widest">{pontoAberto.linha}</span>
              <h2 className="text-2xl font-black italic uppercase tracking-tighter text-white">{pontoAberto.titulo}</h2>
            </div>
            <div className="p-10 overflow-y-auto flex-1 text-center bg-slate-950/10">
              <pre className="whitespace-pre-wrap font-serif text-xl italic leading-relaxed text-slate-100">
                {pontoAberto.letra}
              </pre>
            </div>
          </div>
